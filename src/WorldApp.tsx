...
let count = 0
  function WorldApp() {
    ...
    console.log("WorldApp called from " + (window.location.href || "unknown location"), "count = " + count++);
    ...
    // console.log("The path passed was:", targetPath);
    ...
    // WorldApp called from https://testmain-2n0u5w2p.xyz/
    // WorldApp.tsx:58 WorldApp render called from https://testmain-2n0u5w2p.xyz/
    // WorldApp.tsx:63 WorldApp render path /
    // WorldApp.tsx:65 WorldApp render host testmain-2n0u5w2p.xyz
    // WorldApp.tsx:65 WorldApp render hostname testmain-2n0u5w2p.xyz
    // WorldApp.tsx:67 WorldApp render origin https://testmain-2n0u5w2p.xyz
    // WorldApp.tsx:69 WorldApp render path /
    // WorldApp.tsx:99 

    // hostname looks best

    //  ase the out early.
    // if (hostname === "testmain-2n0u5w2p.xyz" || path.includes("duck")) {
    //   console.log("WorldApp render hostname is testmain-2n0u5w2p.xyz, rendering default app");
    //   return ( // render the this app
    //     <>
    //       <TheDuckApp />
    //     </>
    //   )
    ...
    // else keep looking for another app.
  ...
  // I don't think you can send an offscreenCanvas back to the parent window. You can only send it to a worker. 
  // The parent window can only receive a message with the offscreenCanvas, but it cannot use it directly. It can only send it to a worker.


// App processMessageAsFrame got message courtyard says right back at ya
  ...
  function processMessageFromFrame(event: MessageEvent<any>) {
    ...
    const gl = offscreenCanvas.getContext("webgl2") as WebGL2RenderingContext; //ha, no. We need 3d
    if (!gl) {
      console.error("WorldApp processMessageFromFrame no webgl context in offscreenCanvas", offscreenCanvas);
      return;
    }
  ...
  //   window.add Event Listener("message", processMessageFromFrame, false);
